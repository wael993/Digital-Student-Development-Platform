...
Required: `users`, `refresh_tokens`, `campuses`, `classrooms`, `students`, `student_guardians`, `attendance`, `student_events`, `media`, and later `buses`, `routes`, `activities`, `notifications`.

Not required: `organizations` (the tenant row).

Campuses, classrooms, students, and student–guardian links are created in STUDENT-001. Attendance is created in ATTENDANCE-001. Student journey events are created in JOURNEY-001. Media metadata is created in MEDIA-001.
